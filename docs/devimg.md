...
The AI consent dry-run is a provider setup preview only. The AI review dry-run exercises the OpenAI review artifact path without calling OpenAI. Both require no API key, include no image bytes by default, and write artifacts under `/tmp` for local inspection. Real OpenAI AI review calls are opt-in, require `OPENAI_API_KEY`, and send image bytes only with `--include-images`.

## CI

The main CI workflow uses the public `cleissonom/devimg/action@v0.2.4` Action. It downloads the matching Linux release archive, verifies its checksum, runs `devimg check --fail-on-warning`, validates `devimg manifest export --check`, and uploads the generated review artifact.

After the Action resolves the CLI, CI uses its `binary-path` output to run OpenAI `devimg ai consent --dry-run` and `devimg review --ai --dry-run` previews. The previews are written only under `$RUNNER_TEMP`, asserted nonempty, and do not require `OPENAI_API_KEY`.
